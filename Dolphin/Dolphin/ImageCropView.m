...
static CGFloat const IMAGE_SCALE_FACTOR = 2.9;
float IMAGE_MIN_HEIGHT = 400 / IMAGE_SCALE_FACTOR;
...
- (void)initViews {
    ...
    int initialClearAreaWidth = self.frame.size.width / 5;
    int initialClearAreaHeight = initialClearAreaWidth / IMAGE_SCALE_FACTOR;
    CGPoint centerInView = CGPointMake(self.bounds.size.width / 2, self.bounds.size.height / 2);
    topLeftPoint = [self createControlPointAt:SquareCGRectAtCenter(centerInView.x - initialClearAreaWidth,
                                                                   centerInView.y - initialClearAreaHeight,
                                                                   controlPointSize)];
    
    bottomLeftPoint = [self createControlPointAt:SquareCGRectAtCenter(centerInView.x - initialClearAreaWidth,
                                                                      centerInView.y + initialClearAreaHeight,
                                                                      controlPointSize)];
    
    bottomRightPoint = [self createControlPointAt:SquareCGRectAtCenter(centerInView.x + initialClearAreaWidth,
                                                                       centerInView.y + initialClearAreaHeight, controlPointSize) ];
    
    topRightPoint = [self createControlPointAt:SquareCGRectAtCenter(centerInView.x + initialClearAreaWidth,
                                                                    centerInView.y - initialClearAreaHeight, controlPointSize)];
...
- (CGSize)deriveDisplacementFromDragLocation:(CGPoint)dragLocation draggedPoint:(CGPoint)draggedPoint oppositePoint:(CGPoint)oppositePoint {
    ...
    CGFloat height = width / IMAGE_SCALE_FACTOR;
